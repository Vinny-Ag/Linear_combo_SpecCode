...
def H_e_on_gs_wavefunc(wavefunc,wavefunc_xvals,D,alpha,shift,mu):
    # first interpolate the wavefunction as a spline. Then compute derivative
    spl=interpolate.UnivariateSpline(wavefunc_xvals[:,0],wavefunc[:,0],s=0,k=4)
    second_deriv=spl.derivative(n=2)
    # calculate the 2nd derivative data only at the finite number of x values. 
    second_deriv_dat=second_deriv(wavefunc_xvals[:,0])
    op_wavefunc=np.zeros((wavefunc_xvals.shape[0],2))
    op_wavefunc[:,0]=wavefunc_xvals[:,0]
    op_wavefunc[:,1]=-1.0/(2.0*mu)*second_deriv_dat[:]
    for i in range(op_wavefunc.shape[0]):
        op_wavefunc[i,1]=op_wavefunc[i,1]+(D*(1.0-np.exp(-alpha*(op_wavefunc[i,0]-shift)))**2.0)*wavefunc[i,0]
...
def gs_wavefunc_He_matrix(num_points,start_point,end_point,D_gs,D_ex,alpha_gs,alpha_ex,shift,mu,expansion_number,max_n_gs,x_range,omega_gs):
	overlap_matrix=np.zeros((max_n_gs,max_n_gs))
	Harmonic_LC=compute_wavefunction_n(x_range,start_point,end_point,num_points,omega_gs,D_gs,alpha_gs,mu,expansion_number,0.0)
	func0=Harmonic_LC[:,0].reshape(num_points,1)
	for i in range(max_n_gs):
		# compute gs wavefunction and the action of excited state Hamiltonian He on it.
		func1=Harmonic_LC[:,1+i].reshape(num_points,1)
		He_func1=H_e_on_gs_wavefunc(func1,func0,D_ex,alpha_ex,shift,mu)
		# exploint symmetry
		for j in range(max_n_gs):
			func2=Harmonic_LC[:,1+j].reshape(num_points,1)
			eff_func=func2[:,0]*He_func1[:,1]
			overlap_matrix[i,j]=integrate.simps(eff_func,dx=func2[1,0]-func2[0,0])
			overlap_matrix[j,i]=overlap_matrix[i,j]
...
def Harmonic_expectation_value(wavefunc,D,alpha,shift,mu):
    # first interpolate the wavefunction as a spline. Then compute derivative
    spl=interpolate.UnivariateSpline(wavefunc[:,0],wavefunc[:,1],s=0,k=4)
    second_deriv=spl.derivative(n=2)
    # calculate the 2nd derivative data only at the finite number of x values. 
    second_deriv_dat=second_deriv(wavefunc[:,0])
    bra_op_ket=np.zeros((wavefunc.shape[0],2))
    bra_op_ket[:,0]=wavefunc[:,0]
    bra_op_ket[:,1]=-1.0/(2.0*mu)*second_deriv_dat[:]
    for i in range(bra_op_ket.shape[0]):
        bra_op_ket[i,1]=bra_op_ket[i,1]+(D*(1.0-np.exp(-alpha*(bra_op_ket[i,0]-shift)))**2.0)*wavefunc[i,1]
...
def Harmonic_expansion_matrix(num_points,start_point,end_point,x_range,omega,D,alpha,shift,mu,expansion_number):
    ...
    for i in range(expansion_number):
        ...
        He_func1=Harmonic_expectation_value(func1,D,alpha,shift,mu)
...
def compute_wavefunction_n(x_range,start_point,end_point,num_points,omega,D,alpha,mu,expansion_number,shift):
    expansion_matrix = Harmonic_expansion_matrix(num_points,start_point,end_point,x_range,omega,D,alpha,shift,mu,expansion_number)
    ...
    for i in range (expansion_number):
        for j in range (expansion_number):
            LC_func[:,i,j] = Harm_wavefunc(num_points,start_point,end_point,x_range,omega,mu,j,shift)[:,1]*COEF[j,i]
...
def get_fc_factor(func1,func2,num_points,n_gs,n_ex,HT_q_func):

    counter=0
    if HT_q_func.shape[0]!=num_points: # empty HT func
            for x in func1:
                    x[1+n_gs]=x[1+n_gs]*func2[counter,1+n_ex]
                    counter=counter+1
    else:
            for x in func1:
                    x[1+n_gs]=x[1+n_gs]*func2[counter,1+n_ex]*HT_q_func[counter]
                    counter=counter+1

    return integrate.simps(func1[:,1+n_gs],dx=func1[1,0]-func1[0,0])
...
# VPA new values for harmonic expansion
                self.x_range=np.arange(0,0,1)
                self.expansion_number=expansion_number
		# set derived absorption variables, mainly n_max and freq
                self.set_absorption_variables()
                self.LC_Morse_func1=np.zeros((num_points,expansion_number+1))
                self.LC_Morse_func2=np.zeros((num_points,expansion_number+1))
	...
	# Now declare functions of the Morse oscillator 
        def compute_exact_corr(self,temp,num_points,max_t):
                kbT=const.kb_in_Ha*temp
                self.He_mat=gs_wavefunc_He_matrix(self.grid_n_points,self.grid_start,self.grid_end,self.D_gs,self.D_ex,self.alpha_gs,self.alpha_ex,self.K,self.mu,self.expansion_number,self.n_max_gs,self.x_range,self.freq_gs)
                self.omega_av_qm= compute_omega_av_qm(self.He_mat,self.freq_gs,self.D_gs,kbT)
...
# VPA changed start and end point to be dependent on expansion_number value instead of n_max_gs(ex)
		# now define numerical grid. Find classical turning points
		# on ground and excited state PES
                start_point,end_point=find_classical_turning_points_morse(self.expansion_number,self.expansion_number,self.freq_gs,self.freq_ex,self.alpha_gs,self.alpha_ex,self.D_gs,self.D_ex,self.K)
...
# VPA set up the grid for the Harm_wavefunc function
                # step_x=(end_point-start_point)/self.grid_n_points
...
# VPA
        ...
        def compute_overlaps_and_transition_energies(self):
                self.LC_Morse_func1=compute_wavefunction_n(self.x_range,self.grid_start,self.grid_end,self.grid_n_points,self.freq_gs,self.D_gs,self.alpha_gs,self.mu,self.expansion_number,0.0)
                self.LC_Morse_func2=compute_wavefunction_n(self.x_range,self.grid_start,self.grid_end,self.grid_n_points,self.freq_ex,self.D_ex,self.alpha_ex,self.mu,self.expansion_number,self.K)
                if self.is_HT:
                        ...
                        for i in range(self.n_max_gs):
                                for j in range(self.n_max_gs):
                                    ...
                                    self.wf_overlaps_HT_gs[i,j]=get_fc_factor(self.LC_Morse_func1,self.LC_Morse_func2,self.grid_n_points,i,j,HT_func)
                for i in range(self.n_max_gs):
                        ...
                        for j in range(self.n_max_ex):
                                ...
                                self.wf_overlaps[i,j]=get_fc_factor(self.LC_Morse_func1,self.LC_Morse_func2,self.grid_n_points,i,j,np.zeros(1))
                                self.wf_overlaps_sq[i,j]=self.wf_overlaps[i,j]**2.0
                                # if it is a HT function, also fill HT overlaps:           func1,func2,num_points,n_gs,n_ex,HT_q_func
                                if self.is_HT:
                                    self.wf_overlaps_HT[i,j]=get_fc_factor(self.LC_Morse_func1,self.LC_Morse_func2,self.grid_n_points,i,j,HT_func)
...
class morse_coupled:
        def __init__(self,D_gs,D_ex,alpha_gs,alpha_ex,mu,J,K,E_adiabatic,dipole_mom,max_states_gs,max_states_ex,num_points,num_morse_oscillators,expansion_number,stdout):
                ...
                for i in range(num_morse_oscillators):
                        ...
                        self.morse_oscs.append(morse(D_gs[i],D_ex[i],alpha_gs[i],alpha_ex[i],mu[i],K[i],E_adiabatic,dipole_mom,max_states_gs,max_states_ex,expansion_number,num_points))
                self.num_morse_oscillators=num_morse_oscillators
