...
def Harm_wavefunc(x_range,num_points,start_point,end_point,omega,mu,n,shift):
	step_x=(end_point-start_point)/num_points
	wavefunc=np.zeros((num_points,2))
...
def LC_coefficients(x_range,omega,num_points,start_point,end_point,D,alpha,mu,n,expansion_number,shift):
    ...
    for i in range(expansion_number):
        LC_coeffs[i] = integrate.simps(morse[:,1]*Harm_wavefunc(x_range,num_points,start_point,end_point,omega,mu,i,shift)[:,1],x_range)
    return LC_coeffs

def Linear_combo_wfs(x_range,num_points,start_point,end_point,omega,D,alpha,mu,n,expansion_number,shift):
	step_x=(end_point-start_point)/num_points
	...
	for i in range (expansion_number):
		LC_func[:,i]=Harm_wavefunc(x_range,num_points,start_point,end_point,omega,mu,i,shift)[:,1]*coefficients[i]
	LC_func.sum(axis=1)
	counter = 0
	for i in range (num_points):
...
def LC_func_overlaps(x_range,num_points,start_point,end_point,freq_gs,freq_ex,D_ex,alpha_ex,D_gs,alpha_gs,mu,n_gs,n_ex,expansion_number,shift):
	gs_LC_morse=np.zeros((num_points,n_gs))
	ex_LC_morse=np.zeros((num_points,n_ex))
	gs_ex_Mat=np.zeros((n_gs,n_ex))
	counter=0
	for i in range(n_gs):
		gs_LC_morse[:,i]=Linear_combo_wfs(x_range,num_points,start_point,end_point,freq_gs,D_gs,alpha_gs,mu,i,expansion_number,0.0)[:,1] #start_point+counter*step_x
	print('gs_LC_Morse',gs_LC_morse)
	
	for j in range(n_ex):
		ex_LC_morse[:,j] = Linear_combo_wfs(x_range,num_points,start_point,end_point,freq_ex,D_ex,alpha_ex,mu,j,expansion_number,shift)[:,1]
...
def get_LC_fc_factor(x_range,num_points,expansion_number,start_point,end_point,freq_gs,freq_ex,D_gs,D_ex,alpha_gs,alpha_ex,mu,K,n_gs,n_ex):
        #print start_point,end_point,D,alpha,n_gs,n_ex,d
        func1=Linear_combo_wfs(x_range,num_points,start_point,end_point,freq_gs,D_gs,alpha_gs,mu,n_gs,expansion_number,0.0)
        func2=Linear_combo_wfs(x_range,num_points,start_point,end_point,freq_ex,D_ex,alpha_ex,mu,n_ex,expansion_number,K)
        counter=0
        for x in func1:
                x[1]=x[1]*func2[counter,1]
                counter=counter+1
...
class morse: 
	def __init__(self,D_gs,D_ex,alpha_gs,alpha_ex,mu,K,E_adiabatic,dipole_mom,max_states_gs,max_states_ex,num_points,basis_states):
		...
		self.expansion_number=basis_states
	...
	def set_absorption_variables(self):
		...
		step_x=(self.grid_end-self.grid_start)/self.grid_n_points
		self.x_range = np.arange(self.grid_start,self.grid_end,step_x)
	...
	def compute_LC_exact_response(self,temp,max_t,num_steps):
		kbT=const.kb_in_Ha*temp
		self.compute_LC_overlaps_and_transition_energies()
		self.exact_response_func=compute_exact_response_func(self.wf_overlaps_sq,self.transition_energies,self.freq_gs,self.D_gs,kbT,max_t,num_steps)
	...
	# now compute all wavefunction overlaps and Transition energies 
	def compute_LC_overlaps_and_transition_energies(self):
		for i in range(self.n_max_gs):
			self.gs_energies[i]=compute_morse_eval_n(self.freq_gs,self.D_gs,i)
			for j in range(self.n_max_ex):
				self.ex_energies[j]=compute_morse_eval_n(self.freq_ex,self.D_ex,j)+self.E_adiabatic
				self.transition_energies[i,j]=self.transition_energy(i,j)
				self.wf_overlaps[i,j]=get_LC_fc_factor(self.x_range,self.grid_n_points,self.expansion_number,self.grid_start,self.grid_end,self.freq_gs,self.freq_ex,self.D_gs,self.D_ex,self.alpha_gs,self.alpha_ex,self.mu,self.K,i,j)
				self.wf_overlaps_sq[i,j]=self.wf_overlaps[i,j]**2.0
		print('Transition energies relative to GS')
		print(self.transition_energies[0,:])
...
class morse_list:
	def __init__(self,D_gs,D_ex,alpha_gs,alpha_ex,mu,K,E_adiabatic,dipole_mom,max_states_gs,max_states_ex,num_points,basis_states,num_morse_oscillators,stdout):
		self.morse_oscs = []
		for i in range(num_morse_oscillators):
			self.morse_oscs.append(morse(D_gs[i],D_ex[i],alpha_gs[i],alpha_ex[i],mu[i],K[i],E_adiabatic,dipole_mom,max_states_gs,max_states_ex,num_points,basis_states))
...
# 		self.lambda_0=-get_lambda_0(self.K_emission,self.J_emission,omega_e_sq)
	...
	def compute_total_LC_exact_response(self,temp,max_t,num_steps):
		for i in range(self.num_morse_oscillators):
			self.morse_oscs[i].compute_LC_exact_response(temp,max_t,num_steps)
			print('Computed response func!')
			print(self.morse_oscs[i].exact_response_func)
			if i==0:
				self.total_exact_response_func=self.morse_oscs[i].exact_response_func
...
class morse_coupled:
        def __init__(self,D_gs,D_ex,alpha_gs,alpha_ex,mu,J,K,E_adiabatic,dipole_mom,max_states_gs,max_states_ex,num_points,basis_states,num_morse_oscillators,stdout):
